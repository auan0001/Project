/*
 2/2022 - Modified for use in a temperature measuring project
 conducted by August Forsman (auan(at)mailbox.org).

For the full library, see the contributions made by
...
/* Timing issue when using runtime-bus-selection (!OW_ONE_BUS):
   ...
   but around 14 cycles in configurable bus (us-Delay is 4 cycles per uS) */
...
void ow_temp_rd(char *buffer)
{
  ...
  // Reset, skip ROM, start conversion
  ow_reset();
  ow_byte_wr(OW_SKIP_ROM);
  ow_byte_wr(OW_CONVERTTEMP);

  ow_reset();
  ow_byte_wr(OW_SKIP_ROM);
  ow_byte_wr(OW_RSCRATCHPAD);
